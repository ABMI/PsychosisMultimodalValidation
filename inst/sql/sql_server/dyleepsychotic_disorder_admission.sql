...
select distinct fc.person_id, fc.start_date, fc.end_date
into #final_cohort2
from #final_cohort fc
join (select person_id, note_date, note_title, note_text
from @cdm_database_schema.note
where note_title like @noteTitle1 and person_id in (select distinct person_id from #final_cohort)
) n
on fc.person_id = n.person_id
where n.note_date <= dateadd(dd, 30, start_date) and n.note_date >= dateadd(dd, -30, start_date)


DELETE FROM @target_database_schema.@target_cohort_table where cohort_definition_id = @target_cohort_id;
INSERT INTO @target_database_schema.@target_cohort_table (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
select @target_cohort_id as cohort_definition_id, person_id, start_date, end_date
FROM #final_cohort2 CO
...
TRUNCATE TABLE #final_cohort2;
DROP TABLE #final_cohort2;
